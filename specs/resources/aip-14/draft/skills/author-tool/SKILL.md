...
description:
  ...
  approval class. Implementation lives on a sibling AIP-30 PROVIDER.md;
  this skill optionally chains into the author-provider skill afterwards.
version: 1.0.0
...
inputs:
  ...
  - name: anticipatedProviders
    ...
    required: false
    description:
      Comma-separated list of expected provider kinds (cli, http, mcp, sdk,
      builtin). Used to set `provider_constraints` and `default_provider`.
...
that one or more providers will implement. The skill produces a valid
...
[AIP-30 PROVIDER.md](/docs/aip-30); after authoring the TOOL.md, chain
into the [author-provider skill](../../../aip-30/draft/skills/author-provider/SKILL.md)
to produce one or more PROVIDER.md siblings.
...
- **`requires`**: capability requirements callers must hold. Subfields
  ...
  This is the gate; any provider implementing the tool inherits this
  requirement.
...
- Inputs: be liberal in what you accept; declare optional fields generously.
  Providers narrow via `schema_narrowing.drop_inputs` for what they don't
  support — the contract should carry the union of inputs ANY plausible
  provider needs.
- Outputs: be strict. Required fields are a guarantee every provider must
  satisfy.

### 4. Decide provider routing hints

Two optional fields bias the resolver:

- **`default_provider`**: a provider id the resolver picks when no other
  signal differentiates candidates. Use when one specific implementation
  is the canonical one (paid model, enterprise SLA, etc.).
- **`provider_constraints`**: declare allowed/forbidden provider kinds.
  - `forbid: ["http"]` — for tools handling PII or sensitive data,
    refuse third-party HTTP providers.
...
- **`cost_class`**: trivial / metered / expensive. Providers may
  override; the contract carries the baseline.
- **`timeout_ms`**: contract ceiling. Providers may narrow; never widen.
  Default 30000.
- **`retry`**: contract baseline retry policy
  (`{ max_attempts, backoff, initial_ms }`). Providers may override.
- **`idempotent`**: logical idempotency. Different from network-level
  retry safety (which lives on PROVIDER).

### 6. Add examples

`examples[]` is provider-agnostic — every conformant provider MUST
satisfy these. Author 2-5 input/output pairs covering the canonical
happy paths plus 1-2 edge cases that all providers should handle the
same way (empty input, large input, special characters).

Provider-specific edge cases (DALL-E doesn't support seed, Replicate is
slow on cold starts) live on the PROVIDER's own examples, not here.
...
which used to be valid pre-AIP-30 — now it's a moved-to-PROVIDER signal).

### 8. Author at least one PROVIDER.md sibling

The TOOL.md alone is a contract; without a provider it can't be invoked.
Run the [author-provider skill](../../../aip-30/draft/skills/author-provider/SKILL.md)
with the contract you just authored as input. The skill emits a sibling
PROVIDER.md whose `implements[0].tool` references this TOOL.md.

For a tool that obviously has only one implementation (host-builtin
filesystem, e.g.), produce a single `kind: builtin` PROVIDER.md
co-located in the same folder. For a tool with multiple anticipated
implementations, leave the implementation slot open — providers can
...
import { loadProvider } from "@agstudio/provider-runtime"

const tool = await loadTool("./.tools/pricing-snapshot/TOOL.md")
const provider = await loadProvider("./.providers/apollo-pricing-http/PROVIDER.md")

// host wires tool ↔ provider via the resolver
registry.registerTool(tool)
registry.registerProvider(provider)
```

The tool now appears in the host's catalog, the resolver picks providers
per call, and the audit log records contract + resolved provider on each
invocation.
...
Plus a chain into the author-provider skill that emits one or more:

```
.providers/<provider-id>/
  PROVIDER.md
  SECRETS.md         ← when auth is needed
  provider.ts        ← when custom dispatch is needed
```

When `anticipatedProviders` is provided, the skill kicks off the
author-provider chain immediately for each. Otherwise, the user runs the
chain later when they're ready.

## Common mistakes

- **Adding `entry`, `code`, `run`, `runner`, `secrets`, or `network` to
  TOOL.md.** These belong to PROVIDER.md (per AIP-30). The schema rejects
  them; this skill MUST NOT emit them.
- **Adding `execute` to the `defineTool` call.** `defineTool` no longer
  accepts an `execute` field. Bodies live on `defineProvider().execute`.
- **Approving `auto` with non-empty `mutates`.** Schema rejects this.
  Use `on-mutate` or `always`.
- **Inputs that are provider-specific.** If only one provider supports
  `seed`, declare `seed` as optional on the contract; the other providers
...
- **Forgetting `mutates` declarations.** Every class of mutation any
  provider might perform MUST be declared on the contract. Audit logs
