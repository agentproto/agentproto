...
title: Author a PROVIDER.md (AIP-30)
description:
  Walk through authoring a portable PROVIDER.md manifest — the abstract
...
version: 1.0.0
tags: [aip-30, providers, authoring, manifest, agentproto]
inputs:
  - name: kind
    type: string
    required: true
  ...
  - name: tools
    ...
    required: true
    description:
      Comma-separated list of TOOL.md refs this provider should implement.
...
[AIP-30 PROVIDER.md](/docs/aip-30) manifest plus, when needed, an entry
file that exposes the standard `defineProvider` signature.

## When to use

- "Wrap the OpenAI Images API as an HTTP provider for our image.create tool."
- "Add the stripe CLI as a provider so the billing agent can use it."
- "Connect the filesystem MCP server as a provider for fs.read/write/list."
- "Make the host's builtin fs operations register as a provider."
...
- The user just wants to *call* an existing provider — no authoring needed.
- The user wants to expose a high-level user verb (Create image button,
  ...
  intents route to tools, providers implement tools.
...
| `builtin` | The host runtime provides the implementation natively. | This AIP, § Builtin providers |
...
agent actually invokes. The provider's *internal* dispatch can do
whatever it wants.
...
skill first or have the user clarify what contract this provider should
satisfy.

### 3. Author identity + universal fields

```yaml
spec: agentprovider/v1
...
Most providers need auth; `builtin` and offline tools (`ffmpeg`,
...
env-var binding the provider needs. Reference it from `auth.ref:`.

### 5. Author install paths (CLI / SDK only)

For `kind: cli` or `kind: sdk` providers, identify install methods.
...
For each tool the provider satisfies, author one entry:
...
| `http` | `endpoint: string`, `method: string`, optional `body_template`, `headers` | URL relative to the provider's base URL. |
...
For long-lived providers (HTTP APIs, MCP servers), add a periodic
...
rank multi-provider candidates by cost. Skip when contract's
`cost_class` baseline suffices.

### 10. Validate

Run the manifest through
[`./resources/aip-30/draft/PROVIDER.schema.json`](../../PROVIDER.schema.json):

```bash
ajv validate -s PROVIDER.schema.json -d .providers/<id>/PROVIDER.md \
...
```ts
import { loadProvider, installProvider, verifyProvider, loginProvider } from "@agentproto/provider-runtime"

const provider = await loadProvider("./.providers/openai-images-http/PROVIDER.md")
if (provider.kind === "cli" || provider.kind === "sdk") {
...
The provider now registers in the runtime's catalog, and the resolver
...
```
.providers/<id>/                     ← for kind: http | mcp | sdk | builtin
  PROVIDER.md
  SECRETS.md                         ← unless kind: builtin
  provider.ts                        ← only when custom login/parse needed

.cli/<id>/                           ← for kind: cli (colocated with the bundle)
  CLI.md                             ← which IS a PROVIDER (kind: cli)
  SECRETS.md
...
- **Forgetting to declare `provider_kind` (CLI subtype).** AIP-29
  CLI.md MUST add `provider_kind: cli` to its frontmatter so the
  registry knows the file is both a CLI bundle and a PROVIDER.
