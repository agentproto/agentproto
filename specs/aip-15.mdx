...
| `result` | object \| string | Optional output **value** expression: maps step results to the workflow output using the same reference grammar as a step's `inputs` (`$workflow.inputs.*`, `$steps.<id>.*`, literals). Omitted ⇒ the output is the final step's result. Its shape SHOULD satisfy `outputs`. |
...
`result` uses the step-`inputs` reference grammar (`$workflow.inputs.*`,
...
as a step's `inputs` (`$workflow.inputs.*`, `$steps.<id>.*`, literals;
