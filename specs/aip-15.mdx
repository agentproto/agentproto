...
description: >-
  ...
  in the per-step TOOL.md contracts and their AIP-30 PROVIDER bindings;
...
discussions-to: https://github.com/agentproto/specs/discussions/15
---
...
| Type | Schema |
| Domain | workflows.sh |
| Requires | [AIP-7](/docs/aip-7), [AIP-14](/docs/aip-14) (TOOL), [AIP-16](/docs/aip-16) (IO), [AIP-30](/docs/aip-30) (PROVIDER) |
| Resources | [`./resources/aip-15`](./resources/aip-15/draft) — [`SKILL.md`](./resources/aip-15/draft/skills/author-workflow/SKILL.md), `ADAPTER.md`, `WORKFLOW.schema.json`, `EXAMPLES.md` |
...
companion [`SKILL.md`](./resources/aip-15/draft) generates both `TOOL.md`
...
| `triggers` | object[] | `[]` | Events that start the workflow. See [Triggers](#triggers). |
...
#### Removed (formerly part of AIP-15, moved to per-step TOOL.md + their AIP-30 PROVIDER bindings)

| Field | New home |
|---|---|
| `code` | The TOOL.md a step references is implemented by a PROVIDER which carries `provider.code` (per AIP-26). Workflows have no own bundle. |
| `run` | Same — `provider.run`. |
| `runner` | Same — `provider.runner` (per AIP-17). Per-step provider resolution; workflows don't pin runner. |
| `secrets` | Same — `provider.auth.ref` pointing at SECRETS.md (per AIP-19). |
| `network` | Same — `provider.network.egress`. The egress allowlist is per-provider, not per-workflow. |
| Inline-entry tool steps (`tool: { entry: "./workflow.ts#stepFn" }`) | **Forbidden**. Every `kind: "tool"` step MUST reference an external TOOL.md by id (per AIP-14). Inline bodies break the contract/provider split — the body lives in the provider. Authors who need a one-off body author a sibling TOOL.md + PROVIDER.md. |
...
sandbox all live below in the per-step TOOL.md → PROVIDER chain.
...
- **`secrets`** ([AIP-19](/docs/aip-19)) — env-var bindings:
  vault slugs, OAuth providers, plain values.
...
Run a single tool. The `tool:` field MUST reference an external
TOOL.md by id (per AIP-14). Inline-entry bodies are forbidden — the
body lives on the AIP-30 PROVIDER that implements the TOOL contract,
and provider resolution happens at step-execution time per AIP-30's
resolver.

```yaml
- id: fetch-page
  kind: tool
  tool: pricing-snapshot           # TOOL.md id (REQUIRED — string id, no inline body)
  inputs:
  ...
  approval: auto
  risk_level: 0
  compensation: undo-fetch         # optional
  # Optional per-step provider pin (escape hatch — usually omit):
...
2. Runs the AIP-30 resolver against the call's context to pick a
   PROVIDER (or honours `pinned_provider` when present).
3. Validates `inputs` against the contract's `inputSchema`.
4. Dispatches to the resolved provider's `execute[<toolId>]`.
...
The workflow itself is **provider-agnostic**. The same workflow runs
...
framework, see [`./resources/aip-15/draft/ADAPTER.md`](./resources/aip-15/draft).
...
[`./resources/aip-15/draft/skills/author-workflow/SKILL.md`](./resources/aip-15/draft/skills/author-workflow/SKILL.md) — that an agent
...
6. Validate the manifest against
   [`./resources/aip-15/draft/WORKFLOW.schema.json`](./resources/aip-15/draft)
