...
7. **Suspend points persist.** When a step has `kind: "approval"`,
   the implementation MUST persist enough run state to resume from
   that step on a human decision after process restart. This
   requirement is normative for `kind: "approval"` only — it is NOT
   yet required for `kind: "suspend"`. In-memory-only implementations
   MUST refuse to register suspendable workflows or document the
   limitation explicitly.

   This narrowing codifies the shipped daemon runner's behaviour:
   on reload, a run parked awaiting a human approval is deliberately
   exempt from being failed and its `awaitingApproval` record is
   durable, while any other in-flight run (`running` /
   `awaiting-input`) is marked `failed` with "interrupted by daemon
   restart" — `packages/runtime/src/workflow-runner.ts:335-343`. The
   shipped runner's current behaviour of silently failing a run
   suspended at `kind: "suspend"` on reload is a known defect,
   tracked at https://github.com/agentproto/ts/issues/1195; this
   amendment records what is true today rather than blessing that
   behaviour.
