...
A `partition` is `(asset, restriction, optional narrowed spendableOn, optional
TTL)`, identified `<asset>:<tranche>`. Lots in one partition coalesce freely. A
partition's `spendableOn`, when present, NARROWS the asset's.
