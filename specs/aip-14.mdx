...
status: Draft
type: Schema
ref-impl: https://github.com/agentproto/ts/tree/main/packages/tool
created: 2026-04-28
updated: 2026-04-30
author: Jeremy André <jeremy@agentik.net>
requires: [1, 2, 7, 16, 30]
resources: ./resources/aip-14/draft
discussions-to: https://github.com/agentproto/agentproto/discussions/14
layer: capabilities
package: "@agentproto/tool"
---
...
| Type | Schema |
| Domain | tools.sh |
| Requires | [AIP-7](/docs/aip-7) (governance), [AIP-16](/docs/aip-16) (IO), [AIP-30](/docs/aip-30) (DRIVER) |
| Implemented by | Any [AIP-30 DRIVER](/docs/aip-30) declaring the tool in its `implements[]` |
| Resources | [`./resources/aip-14`](https://github.com/agentproto/agentproto/tree/main/specs/resources/aip-14/draft) — [`SKILL.md`](https://github.com/agentproto/agentproto/blob/main/specs/resources/aip-14/draft/skills/author-tool/SKILL.md), `ADAPTER.md`, `TOOL.schema.json`, `EXAMPLES.md` |
...
all live on the [AIP-30 DRIVER](/docs/aip-30) layer.

A single TOOL.md MAY be implemented by zero, one, or many drivers
— the registry's runtime resolves driver per call based on
...
1. **Driver swap stops being a refactor.** Today a tool that calls
   ...
   contract/driver split, swapping drivers is editing a config:
   the tool stays unchanged, a new DRIVER.md declares it implements
   the same contract.

2. **Multi-driver routing becomes data.** "Pick the cheapest
   driver that satisfies workspace policy and supports my style
...
3. **Governance gates the contract, not the implementation.**
   ...
   contract properties enforced regardless of which driver serves
   the call. An OpenAI driver can't downgrade approval below what
   the contract declares.

4. **Catalogs render from data.** "What can this agent do?" is
   answerable by walking the TOOL.md registry. Each tool's matching
   drivers are derived from the resolver's filter chain, so the
   catalog can show "create-image (3 drivers, 1 unauthed)" without
   reading any code.

5. **Testing decouples.** The contract has its own examples (input/
   output pairs that every driver MUST satisfy). Each driver may
   add its own examples (driver-specific edge cases). The two
   layers test independently.

`TOOL.md` is the contract layer. [AIP-30 DRIVER](/docs/aip-30) is
...
1. **Contract before implementation.** The manifest declares what
   ...
   is informational prose; the executable body lives on a DRIVER
...
2. **Side effects are first-class.** Every tool declares its
   ...
   contract — no separate audit metadata, no per-driver
   re-declaration.
...
4. **Drivers narrow contracts; widening is forbidden.** A driver
   ...
   calls using dropped inputs). A driver MUST NOT add inputs the
...
6. **Composable with siblings.** `TOOL.md` may live alone in a tool
   folder, alongside driver folders that implement it, or in a
   shared catalog directory. The manifest is the index; driver
...
Tools live in a single folder, separate from the drivers that
implement them:
...
A tool MAY co-exist with a single driver in the same folder when
...
make the multi-driver layout explicit and easy to scan.
...
| `requires` | object | `{}` | Capability requirements (governance gating per [AIP-7](/docs/aip-7)). Subfields: `network: string[]`, `secrets: string[]`, `tools: string[]` (other tool ids it calls). Drivers MAY add to this set; MUST NOT remove from it. |
...
| `default_implementation` | string \| null | null | Optional id of an implementation (a [DRIVER.md](/docs/aip-30) or [CODE.md](/docs/aip-26)) to use when the resolver has no other signal. Lets authors pin the canonical implementation while still permitting overrides via `context.pinnedProvider`. Null = pick by resolver policy. (Renamed from `default_driver` for terminological consistency with the post-AIP-26 model where CODE.md can also implement TOOL contracts.) |
| `implements` | string \| object \| null | null | Optional [AIP-39 ACTION](/docs/aip-39) reference declaring which abstract verb this tool implements. When set, the tool INHERITS the action's `mutates`, `risk_level`, `approval`, `requires` (with narrowing rules — see "Action implementation" below). [POLICY.md (AIP-38)](/docs/aip-38) grants on actions, not tools, so multiple TOOLs implementing the same action share one grant. Format: bare action id (`"storage:commit"`), registry ref (`"@agentik/actions/standard/storage-commit"`), workspace path (`"./actions/storage-commit/ACTION.md"`), or inline action definition (`{ inline: { id: "...", ... } }`). |
...
shape MUST move these fields to a DRIVER.md sibling.
...
- **`## Errors`** — error classes the tool can return + how callers
  should handle them.
- **`## Driver notes`** — anticipated drivers, known schema
...
Hosts MUST wrap driver runtime errors into this envelope. The
...
A tool MUST declare every class of mutation any of its drivers
might perform. A driver whose body observably writes resources
...
[AIP-39 ACTION.md](/docs/aip-39), the tool joins the action's
...
**Inheritance.** At parse time, the host loads the resolved action
and merges its fields into the TOOL view:
...
**Narrowing rule, formally**: a TOOL widens an action when its
declaration would relax any constraint the action set as the floor.
Widening is rejected at parse time:
...
2. **No `execute` on the contract.** `defineTool` MUST NOT accept an
   `execute` field. Bodies live on DRIVER (per AIP-30); hosts
...
4. **`contextSchema` is contract obligation.** When declared,
   every conformant driver MUST accept the same context shape.
   Drivers MAY narrow via their own internal validation, but the
   contract's `contextSchema` is the floor.

5. **`mutates` is the truth.** If any driver's body observably
...
6. **No I/O at module load.** The module containing `defineTool`
   ...
   contract should perform I/O — bodies do that on DRIVER, never
   here.
...
[`./resources/aip-14/draft/ADAPTER.md`](https://github.com/agentproto/agentproto/blob/main/specs/resources/aip-14/draft/ADAPTER.md).
...
[`./resources/aip-14/draft/skills/author-tool/SKILL.md`](https://github.com/agentproto/agentproto/blob/main/specs/resources/aip-14/draft/skills/author-tool/SKILL.md) —
...
5. Identify the driver(s) that will implement this tool. For
   each, run the [AIP-30 author-driver skill](/docs/aip-30) to
   produce a sibling DRIVER.md.
6. Validate the manifest against
   [`./resources/aip-14/draft/TOOL.schema.json`](https://github.com/agentproto/agentproto/tree/main/specs/resources/aip-14/draft).
...
1. Author a sibling DRIVER.md per [AIP-30](/docs/aip-30) carrying
   ...
   - `secrets` (per-manifest bindings) → `driver.auth.ref` pointing
     at a `SECRETS.md`
   - `network` → `driver.network.egress`
   - `entry`'s `execute` body → `driver.execute[<toolId>]`
2. Set `driver.implements[0].tool` to the (now-amputated) TOOL.md.
3. Remove the moved fields from TOOL.md; add `default_implementation`
   pointing at the new DRIVER.md's `id`.
...
1. Add `TOOL.md` for the contract: id, schemas, mutates, approval,
   requires.
2. Add `DRIVER.md` for the implementation: kind, auth, sandbox,
...
enforcement at the driver level). [AIP-7](/docs/aip-7)'s
...
1. **Streaming outputs.** Tools that yield progressive results — do
   ...
   driver-kind-specific extensions? Defer until concrete
   streaming drivers ship enough to settle the abstraction.

2. **Cost reporting.** Should `cost_class` evolve into a numeric
   `cost_units` field tied to a per-host meter? Today drivers
   override with `cost_units_per_call`; the contract baseline could
   match.

3. **Schema-narrowing at the contract level.** Currently
   `schema_narrowing` is a driver-side declaration. Should the
   contract also be able to declare which inputs are
   "optional-but-most-drivers-support" vs "optional-and-rare"?
   The resolver could prefer drivers covering more optional
...
4. **Per-tool budget overrides.** A workspace might want to cap
   ...
   Tool's `id`? Driver's `quota_key`? Both?

These remain open until enough adapters and drivers ship to
...
- [AIP-30 — DRIVER.md](/docs/aip-30) — implementation layer
  binding contracts to backends
- [AIP-38 — POLICY.md](/docs/aip-38) — grants on the actions tools implement
- [AIP-39 — ACTION.md](/docs/aip-39) — abstract verb tools implement (`implements: <action-ref>`)
- [`./TOOL.schema.json`](https://github.com/agentproto/agentproto/tree/main/specs/resources/aip-14/draft) — schema validator
- [`./ADAPTER.md`](https://github.com/agentproto/agentproto/tree/main/specs/resources/aip-14/draft) — implementer's guide
