...
| `default_implementation` | string \| null | null | Optional id of an implementation (a [DRIVER.md](/docs/aip-30) or [CODE.md](/docs/aip-26)) to use when the resolver has no other signal. Lets authors pin the canonical implementation while still permitting overrides via `context.pinnedProvider`. Null = pick by resolver policy. (Renamed from `default_implementation` for terminological consistency with the post-AIP-26 model where CODE.md can also implement TOOL contracts.) |
