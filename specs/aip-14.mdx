...
| `default_driver` | string \| null | null | Optional `driver.id` to use when the resolver has no other signal. Lets authors pin the canonical implementation while still permitting overrides via `context.pinnedProvider`. Null = pick by resolver policy. |
...
default_driver: apollo-pricing-http
...
Driver routing: default to Apollo (`default_driver`); fall through
...
3. Remove the moved fields from TOOL.md; add `default_driver`
