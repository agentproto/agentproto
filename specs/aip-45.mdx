...
9. When `protocol: "print"`, the manifest MAY omit the `print` block
   (Claude Code defaults apply); when `print.event_schema` is
   declared, it MUST be one of the closed enum values listed in
   § The `print` arm below.
...
> **Amendment — codifies shipped behaviour.** `protocol: "print"` is a
> fourth discriminator arm. It is validated by the closed enum
> `z.enum(["acp", "mcp", "proprietary", "print"])` in
> `packages/driver/agent-cli/src/schema.ts` and driven by
> `packages/driver/agent-cli/src/protocol/print-arm.ts` in the
...
- `Session.close()` — tear down the session and (if last) the
  subprocess.
- Protocol arms under `src/protocol/{acp,mcp,proprietary,print}.ts`.
  The ACP arm delegates to `@agentproto/acp` (AIP-44).
