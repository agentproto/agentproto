...
contract resolves `suspended { reason: "input-required" }` **only** on
an explicit signal — never by reading the turn's final message as
prose. There are exactly two explicit signals:
