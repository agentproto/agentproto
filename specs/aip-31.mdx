---
title: "AIP-31: HTTP.md — agenthttp/v1 (HTTP driver specialisation)"
description: >-
  A markdown + frontmatter format for declaring an HTTP-API driver — its base URL,
  per-tool endpoint/method/body templates, header bindings, response extraction, and
  (when needed) streaming hints. Specialises AIP-30 DRIVER for the `kind: http` case.
  Most third-party APIs (OpenAI, Stripe, Replicate, Anthropic) wrap as HTTP drivers.
aip: 31
status: Draft
type: Schema
driver_kind: http
...
ref-impl: https://github.com/agentproto/ts/tree/main/packages/driver/http
resources: ./resources/aip-31/draft
discussions-to: https://github.com/agentproto/agentproto/discussions/31
layer: drivers
package: "@agentproto/driver-http"
---
...
| Resources | [`./resources/aip-31`](https://github.com/agentproto/agentproto/tree/main/specs/resources/aip-31/draft) — `HTTP.schema.json`, [`ADAPTER.md`](https://github.com/agentproto/agentproto/blob/main/specs/resources/aip-31/draft/ADAPTER.md), [`EXAMPLES.md`](https://github.com/agentproto/agentproto/blob/main/specs/resources/aip-31/draft/EXAMPLES.md), [`SKILL.md`](https://github.com/agentproto/agentproto/blob/main/specs/resources/aip-31/draft/skills/author-http/SKILL.md) |

## Abstract

`HTTP.md` is the `kind: http` specialisation of
[AIP-30 DRIVER](/docs/aip-30). It packages an HTTP-API integration —
...
HTTP-driver format, every integration ships its own auth/parse/retry
...
2. **Auth inconsistency.** Bearer tokens, API-key-in-header,
   ...
   each handled bespokely. The driver's `auth.login` and the HTTP
...
1. **Inherit, don't repeat.** Identity, auth, network, region,
   policy, region, cost-override all live on DRIVER. HTTP only adds
...
2. **Declarative over imperative.** Body templates use `${input.X}`
   ...
   the driver's optional entry, not in the manifest.

3. **One base_url per driver.** A single HTTP driver serves one
   API host. Cross-host drivers (multi-region failover) are two
   drivers, not one with conditional URL.
...
4. **Standard auth shapes.** Bearer-in-header is default. API-key-in-
   ...
   the DRIVER's `auth.login` with custom callback handling in the
   entry.
...
HTTP drivers live in a single folder, separate from the TOOL.md
...
[AIP-30 DRIVER.md](/docs/aip-30) plus the HTTP-specific fields
below.
...
The OAuth refresh flow lives in the driver's optional `driver.ts`
...
section) AND the driver implements streaming, declare:
...
1. **Inherit AIP-30 conformance.** All AIP-30 `defineDriver` rules
   apply. Identity-vs-frontmatter, signal honouring, sandbox
   enforcement, no module-load I/O.

2. **`base_url` is single-host.** A driver serves one host. Multi-
   region failover is multiple drivers, not conditional URL.
...
5. **Streaming is opt-in.** Tools whose contract doesn't declare
   streaming-eligible outputs cannot stream regardless of driver's
   streaming declaration.

## Authoring with `SKILL.md`

The canonical way to *generate* an HTTP `DRIVER.md` is via the paired
[author-http skill](https://github.com/agentproto/agentproto/blob/main/specs/resources/aip-31/draft/skills/author-http/SKILL.md).
The skill walks the agent through:

1. Pick `id` (kebab + `-http` suffix).
2. Identify `base_url`.
3. Identify auth scheme + map to `default_headers` / `auth.state.env`.
4. For each TOOL the driver implements: identify endpoint, method,
   body shape, response shape; author `metadata.http`.
5. Declare `network.egress` (the API's host).
6. Validate against
   [`./resources/aip-31/draft/HTTP.schema.json`](https://github.com/agentproto/agentproto/tree/main/specs/resources/aip-31/draft).

## Example

See [`./resources/aip-31/draft/EXAMPLES.md`](https://github.com/agentproto/agentproto/blob/main/specs/resources/aip-31/draft/EXAMPLES.md)
...
This AIP is a strict specialisation. Every HTTP driver IS a
DRIVER. The AIP-30 schema validates the universal fields; the
...
- **Idempotency keys** prevent retry-induced double-charges. When
  the contract is `mutates: ["external:*"]`, drivers SHOULD declare
...
- **TLS verification** is non-negotiable. Hosts MUST validate
  certificates; drivers MUST NOT request TLS-skip-verify.
...
4. **WebSocket transports.** Some APIs (Anthropic's streaming
   ...
   AIP-XX WebSocket driver.

## See also

- [AIP-14 — TOOL.md](/docs/aip-14) — abstract contracts HTTP drivers
  implement
...
- [`./HTTP.schema.json`](https://github.com/agentproto/agentproto/tree/main/specs/resources/aip-31/draft) — JSON Schema validator
- [`./ADAPTER.md`](https://github.com/agentproto/agentproto/blob/main/specs/resources/aip-31/draft/ADAPTER.md) — implementer's guide
- [`./EXAMPLES.md`](https://github.com/agentproto/agentproto/blob/main/specs/resources/aip-31/draft/EXAMPLES.md) — additional patterns
