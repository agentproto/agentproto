...
| `signature.evidence.target` (when applicable) | `RefIn<"file">` | typed-name evidence references no target; esign-external evidence MAY carry a `Ref<url>` to the provider envelope |
