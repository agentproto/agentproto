...
ref-impl: https://github.com/agentproto/ts/tree/main/packages/driver/cli
resources: ./resources/aip-29/draft
discussions-to: https://github.com/agentproto/agentproto/discussions/29
layer: drivers
package: "@agentproto/driver-cli"
---
...
| Resources | [`./resources/aip-29`](https://github.com/agentproto/agentproto/tree/main/specs/resources/aip-29/draft) — `CLI.schema.json`, [`ADAPTER.md`](https://github.com/agentproto/agentproto/blob/main/specs/resources/aip-29/draft/ADAPTER.md), [`EXAMPLES.md`](https://github.com/agentproto/agentproto/blob/main/specs/resources/aip-29/draft/EXAMPLES.md), [`SKILL.md`](https://github.com/agentproto/agentproto/blob/main/specs/resources/aip-29/draft/skills/author-cli/SKILL.md) |
...
**`defineCliDriver(...)`**, whose signature any implementation in any
...
1. **Sidecar daemons** — some CLIs ship a binary that depends on a
   long-running companion process (`openclaw onboard --install-daemon`,
   Ollama's model server, n8n's queue worker). The binary is on
   `$PATH` after `install`, but the bundle is not actually usable
   until the daemon is up.

2. **Bind-time configuration** — values the user picks once and
   reuses (Discord channel, Slack workspace, gateway URL, default
   model). These aren't auth refreshes; they're *configuration*
   captured at first run and persisted into the bundle's secrets
   store or an env-var slot the runner injects on every spawn.
...
| `oauth`    | Drive an [AIP-19](/docs/aip-19) OAuth flow. Tokens land in the named SECRETS slug. | `secret_slug` |
...
- `id` (REQUIRED) — stable per-step identifier. Appears in audit logs
  and the host's setup ledger.
- `description` (optional) — surface adapter shows this above the
  prompt or progress line.
- `skip_if` (optional) — `{ cmd, exit_code? }`. When the cmd exits
  with the matching code, the step is skipped. Lets `agentproto
  setup <slug>` re-runs be idempotent without external state.
- `persist` (optional, applies to `cmd` / `prompt` / `external`) —
  where the captured value lands. **Exactly one** of:
  - `env: <NAME>` — env var the runner injects on every spawn
    (lifted from the workspace secrets store).
  - `secret_slug: <slug>` — write into an AIP-19 secret slot.
  - `cmd: "<…> ${value}"` — run a CLI-specific config command with
    the captured value substituted.
...
Steps run in declared order. A failure aborts the pipeline and
...
- **CLI surface** — text/secret/select prompts via inquirer-like
  TUI. `external` opens the URL via `open`/`xdg-open` and polls the
  callback URL.
- **Web surface** — modal with form inputs. `external` redirects
  in-tab and reads the callback via `postMessage` or a host-managed
  callback page.
- **Headless surface** — surfaces `setup_required` to the caller
  with the pending step list; refuses to spawn the bundle until the
  caller has driven setup separately.
...
## The `defineCliDriver` standard signature

> **Amendment — codifies shipped behaviour.** The canonical entry
> point ships as **`defineCliDriver`** in
> [`packages/driver/cli/src/index.ts`](https://github.com/agentproto/ts/tree/main/packages/driver/cli)
> (`@agentproto/driver-cli`): sugar over [AIP-30](/docs/aip-30)'s
> `defineDriver` with `kind: "cli"` and an auto-synthesised per-tool
> `execute` map. No `defineCli` alias is exported; earlier drafts
> named the entry `defineCli` — the shipped name is normative.

Every implementation that consumes `CLI.md` and ships routing
helpers MUST expose a function named **`defineCliDriver`** whose
signature matches the contract below. Many bundles ship a
frontmatter-only manifest with no entry; this signature is for
entries that need custom login/output adapters or non-default
spawn/output conventions.

### Signature (TypeScript notation, normative)

```ts
defineCliDriver(definition: CliDriverDefinition): DriverHandle

interface CliDriverDefinition extends Omit<DriverDefinition, "kind" | "execute"> {
  // Identity + lifecycle + adapters — inherited from the AIP-30
  // DriverDefinition base: id, name, description, version,
  // implements[], install, versionCheck, network, tags, metadata,
  // login, refresh, parseOutput, detectExpiry, ...

  // CLI-specific surface:
  kind?: "cli"                 // forced to "cli" by defineCliDriver
  bin: string                  // binary name on $PATH, or workspace-relative path
  binArgs?: readonly string[]  // default argv prefix injected before per-tool argv
  output?: {                   // output parsing convention
  ...
  sandbox?: {                  // declaration only; enforcement is host policy
...
`defineCliDriver` synthesises the `execute` map: one body per
`implements[]` entry, built from `bin` / `binArgs` / `output` — argv
templating with `${input.X}` substitution and filters (`default`,
`optional`, `flag`), exit-code → semantic-error mapping, and
text/JSON output parsing.

The login / refresh / parseOutput / detectExpiry adapters and their
argument shapes are inherited unchanged from AIP-30's
`DriverDefinition`; the shapes below remain normative for CLI
implementations:
...
1. **One canonical name.** The exported name MUST be
   `defineCliDriver`. Implementations MAY also re-export under
   host-specific aliases (`createCli`, `cli`) but the canonical name
   is what `CLI.md` adapters reference.
...
5. **Sandbox enforcement is the host's.** The bundle declares
   policy; the host enforces it. A `defineCliDriver` implementation MUST
   NOT subvert host enforcement (e.g. by spawning child processes
   when `exec.allow: false`).

6. **No I/O at module load.** The module containing `defineCliDriver`
   MUST be safely importable as a side-effect-free unit. All I/O
   happens inside `login` / `refresh` / `parseOutput`.

### Implementer's guide

For step-by-step guidance on building a `defineCliDriver`-conformant
implementation in a specific language or framework, see
[`./resources/aip-29/draft/ADAPTER.md`](https://github.com/agentproto/agentproto/blob/main/specs/resources/aip-29/draft/ADAPTER.md).
...
[`./resources/aip-29/draft/skills/author-cli/SKILL.md`](https://github.com/agentproto/agentproto/blob/main/specs/resources/aip-29/draft/skills/author-cli/SKILL.md) —
...
7. Validate the manifest against
   [`./resources/aip-29/draft/CLI.schema.json`](https://github.com/agentproto/agentproto/tree/main/specs/resources/aip-29/draft).
...
3. Move auth / sandbox / output-parsing logic into the
   `defineCliDriver` entry so it's reused across subcommands.
...
- [`./CLI.schema.json`](https://github.com/agentproto/agentproto/tree/main/specs/resources/aip-29/draft) — JSON Schema validator
- [`./ADAPTER.md`](https://github.com/agentproto/agentproto/blob/main/specs/resources/aip-29/draft/ADAPTER.md) — implementer's guide
- [`./EXAMPLES.md`](https://github.com/agentproto/agentproto/blob/main/specs/resources/aip-29/draft/EXAMPLES.md) — additional CLI.md examples
- [AIP-45 — AGENT-CLI.md](/docs/aip-45) — sibling spec for **interactive
  agent CLIs** (Hermes, Claude Code, OpenCode, Goose, Gemini CLI). Shares
  AIP-29's `installMethod` / `versionCheck` / `auth` blocks via JSON
  Schema `$ref`; covers the bidirectional, persistent-session shape that
  AIP-29's one-shot `cmd → output` model isn't designed for.
