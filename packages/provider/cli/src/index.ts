 ...
 * sandbox in PROVIDER.md frontmatter; v0.2 will wire enforcement.
 *
...
/**
 * Define a CLI provider — sugar over defineProvider with kind: cli
 * and an auto-synthesised execute map.
 */
export function defineCliProvider(definition: CliProviderDefinition): ProviderHandle {
...
function createExecuteFn(args: {
  ...
  bin: string
  binArgs: readonly string[]
  output: NonNullable<CliProviderDefinition["output"]>
  sandbox: CliProviderDefinition["sandbox"]
}): ExecuteFn {
...
async function runSubprocess(args: {
  ...
  return await new Promise((resolve, reject) => {
    ...
    child.on("error", err => {
      ...
      reject(new CliProviderError("upstream_error", err.message))
    })
...
function parseOutput(args: {
  ...
  if (semantic === "auth_required") {
    throw new CliProviderError("auth_required", args.stderr.trim() || "auth required")
  }
  if (semantic === "rate_limited" || semantic === "rate-limited") {
    throw new CliProviderError("rate_limited", args.stderr.trim() || "rate limited", true)
  }
  throw new CliProviderError(
...
function buildEnv(
  envConfig: NonNullable<CliProviderDefinition["sandbox"]>["env"] | undefined,
