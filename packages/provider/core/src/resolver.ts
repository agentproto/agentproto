 ...
 *           respecting tool.providerConstraints (forbid / requireKind)
 ...
 *  Phase 5 — Cost / preference rank: default_provider first, else by
...
export function resolveProvider(args: {
  ...
  const phase1: Array<{ provider: ProviderHandle; entry: ImplementsEntry }> = []
  for (const provider of candidates) {
    ...
    if (tool.providerConstraints.forbid.includes(provider.kind)) {
      rejected.push({ providerId: provider.id, phase: 1, reason: "kind_forbidden_by_contract" })
      continue
    }
    if (
      tool.providerConstraints.requireKind.length > 0 &&
      !tool.providerConstraints.requireKind.includes(provider.kind)
    ) {
  ...
  // default_provider preference.
  if (tool.defaultProvider) {
    const def = phase3.find(({ provider }) => provider.id === tool.defaultProvider)
    if (def) {
