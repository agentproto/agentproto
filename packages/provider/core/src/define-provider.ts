...
/**
 * AIP-30 reference implementation of `defineProvider`.
 *
 * Returns a {@link ProviderHandle} with defaults applied. The resolver
 * (`resolveProvider`) and runners (`runTool`) consume this shape and
 ...
 *  - No I/O at module load — `defineProvider(...)` is pure construction.
 *
...
export function defineProvider(
  ...
  if (!ID_RE.test(definition.id)) {
    throw new Error(
      `defineProvider: invalid id '${definition.id}' — must match ${ID_RE}`
    )
  }
  if (!definition.description || definition.description.length > 2000) {
    throw new Error(
      `defineProvider: id='${definition.id}' description must be 1–2000 chars`
    )
  }
  if (definition.implements.length === 0) {
    throw new Error(
      `defineProvider: id='${definition.id}' must declare ≥1 implements[] entry`
    )
  }
  ...
  for (const impl of definition.implementations ?? []) {
    ...
    if (executeMap[id] && executeMap[id] !== typedBody) {
      ...
      throw new Error(
        `defineProvider: id='${definition.id}' has duplicate body for '${id}' — ` +
  ...
  for (const toolId of declaredToolIds) {
    if (!executeKeys.has(toolId)) {
      throw new Error(
        `defineProvider: id='${definition.id}' implements '${toolId}' but no execute['${toolId}'] body provided`
      )
    }
  }
  for (const key of executeKeys) {
    if (!declaredToolIds.has(key)) {
      throw new Error(
        `defineProvider: id='${definition.id}' has execute['${key}'] but '${key}' is not in implements[]`
      )
    }
  }

  const handle: ProviderHandle = Object.freeze({
    id: definition.id,
