 ...
 *
 * Sugar over `defineProvider({ kind: "http", ... })` plus the
...
export interface HttpProviderDefinition extends Omit<ProviderDefinition, "kind" | "execute"> {
  kind?: "http"
...
export interface BuildRequestArgs {
  ...
  providerCtx: { secrets: Record<string, string>; baseUrl: string; [k: string]: unknown }
  signal: AbortSignal
}
...
/**
 * Define an HTTP provider — sugar over defineProvider with kind: http
...
export function defineHttpProvider(
  definition: HttpProviderDefinition
): ProviderHandle {
...
function createExecuteFn(args: {
  ...
  buildRequest?: HttpProviderDefinition["buildRequest"]
  parseResponse?: HttpProviderDefinition["parseResponse"]
}): ExecuteFn {
  ...
  return async ({ input, context, providerCtx, signal }) => {
    ...
    if (args.buildRequest) {
      request = await args.buildRequest({
        ...
        input: inputObj,
        context: ctxObj,
        providerCtx: { ...provCtx, baseUrl: args.baseUrl },
        signal,
      })
    } else {
    ...
    if (args.parseResponse) {
      ...
      throw new HttpProviderError(parsed.error.code, parsed.error.message, parsed.error.retryable)
    }

    if (response.status === 401 || response.status === 403) {
      throw new HttpProviderError("auth_required", `HTTP ${response.status}`, false)
    }
    if (response.status === 429) {
      throw new HttpProviderError("rate_limited", "HTTP 429", true)
    }
    if (response.status >= 500) {
      throw new HttpProviderError("upstream_error", `HTTP ${response.status}`, true)
    }
    if (response.status >= 400) {
      throw new HttpProviderError("upstream_error", `HTTP ${response.status}`, false)
    }
